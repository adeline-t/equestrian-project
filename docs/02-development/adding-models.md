...
```javascript
import { getDatabase, handleDbError, jsonResponse, validateEmail, validatePhone, validateRequired } from '../db.js';
    ...
    try {
        ...
        if (method === 'PUT' && pathParts.length === 3) {
            ...
            const updateData = {
                ...
                hourly_rate: body.hourly_rate ? parseFloat(body.hourly_rate) : currentInstructor.hourly_rate,
                max_students: body.max_students ? parseInt(body.max_students) : currentInstructor.max_students,
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            // ... existing riders, horses, associations routes ...
                    ...
                    {instructors.map((instructor) => (
                        <div key={instructor.id} className="instructor-card">
                            <div className="card-header">
                                ...
                                <span className={`badge ${isInstructorActive(instructor) ? 'badge-success' : 'badge-secondary'}`}>
                            ...
                            <div className="card-body">
                                ...
                                {instructor.hourly_rate && (
                                    <div className="info-item">
                                        💰 {instructor.hourly_rate}€/heure
                                    </div>
                                )}
                                {instructor.max_students && (
                                    <div className="info-item">
                                        👥 Max: {instructor.max_students} élèves
                                    </div>
                                )}
