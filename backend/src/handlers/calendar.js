...
import { generateLessonInstances, generateUpcomingInstances, calculateOccurrences } from '../services/lesson-generator.js';
...
export async function handleCalendar(request, env) {
    ...
    const lessonRepo = new LessonRepository(env.DB);
...
async function createLesson(request, env, lessonRepo) {
    ...
    // Validation
    if (!data.name || !data.lesson_type || !data.lesson_date || !data.start_time || !data.duration_minutes) {
        return jsonResponse({
            success: false,
