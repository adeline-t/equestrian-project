...
// .wrangler/tmp/bundle-6xaeiz/checked-fetch.js
...
// .wrangler/tmp/bundle-6xaeiz/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-6xaeiz/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
function handleDbError(error) {
  ...
  if (error.code === "PGRST116") {
    return jsonResponse({ error: "Enregistrement non trouv\xE9" }, 404);
  }
  if (error.code === "23505") {
    return jsonResponse({ error: "Cet enregistrement existe d\xE9j\xE0" }, 409);
  }
  if (error.code === "23503") {
    return jsonResponse({ error: "R\xE9f\xE9rence invalide" }, 400);
  }
  return jsonResponse(
    { error: "Erreur de base de donn\xE9es" },
...
function checkRateLimit(clientId, limit = 100, windowMs = 6e4) {
  const now = Date.now();
...
// src/handlers/riders.js
async function handleRiders(request, env) {
  const db = getDatabase(env);
  const url = new URL(request.url);
  const pathParts = url.pathname.split("/").filter(Boolean);
  const clientIP = request.headers.get("CF-Connecting-IP") || "unknown";
  if (!checkRateLimit(clientIP, 60, 6e4)) {
    return jsonResponse({ error: "Trop de requ\xEAtes" }, 429);
  }
  ...
  try {
    ...
    if (request.method === "POST" && pathParts.length === 2) {
      ...
      if (!body) {
        return jsonResponse({ error: "Corps de requ\xEAte invalide" }, 400, getSecurityHeaders());
      }
      ...
      if (body.phone && !validatePhone(body.phone)) {
        return jsonResponse({ error: "Format de t\xE9l\xE9phone invalide" }, 400, getSecurityHeaders());
      }
      const riderData = {
        name: body.name.trim(),
    ...
    if (request.method === "PUT" && pathParts.length === 3) {
      ...
      if (isNaN(riderId) || !body) {
        return jsonResponse({ error: "ID ou corps de requ\xEAte invalide" }, 400, getSecurityHeaders());
      }
      if (body.email && !validateEmail(body.email)) {
        return jsonResponse({ error: "Format d'email invalide" }, 400, getSecurityHeaders());
      }
      ...
      const { data, error } = await db.from("riders").update(updateData).eq("id", riderId).select().single();
      if (error) return handleDbError(error);
      return jsonResponse(data, 200, getSecurityHeaders());
    }
    if (request.method === "DELETE" && pathParts.length === 3) {
      ...
      if (error) return handleDbError(error);
      return jsonResponse({ message: "Cavalier supprim\xE9 avec succ\xE8s" }, 200, getSecurityHeaders());
    }
    return jsonResponse({ error: "Route non trouv\xE9e" }, 404, getSecurityHeaders());
  } catch (error) {
    console.error("Unexpected error:", error);
    return jsonResponse({ error: "Erreur serveur interne" }, 500, getSecurityHeaders());
  }
}
__name(handleRiders, "handleRiders");
async function handleRiderHorses(request, env, riderId) {
  const db = getDatabase(env);
  const clientIP = request.headers.get("CF-Connecting-IP") || "unknown";
  if (!checkRateLimit(clientIP, 60, 6e4)) {
    return jsonResponse({ error: "Trop de requ\xEAtes" }, 429, getSecurityHeaders());
  }
  try {
    ...
    const { data, error } = await db.from("rider_horse_associations").select(`
        id,
      ...
      `).eq("rider_id", riderIdNum).order("association_start_date", { ascending: false });
    if (error) return handleDbError(error);
    return jsonResponse(data, 200, getSecurityHeaders());
  } catch (error) {
    console.error("Unexpected error:", error);
...
__name(handleRiderHorses, "handleRiderHorses");

// src/handlers/horses.js
init_checked_fetch();
init_modules_watch_stub();
async function handleHorses(request, env) {
  const db = getDatabase(env);
  const url = new URL(request.url);
  const pathParts = url.pathname.split("/").filter(Boolean);
  const clientIP = request.headers.get("CF-Connecting-IP") || "unknown";
  ...
  try {
    ...
    if (request.method === "POST" && pathParts.length === 2) {
      ...
      if (!["horse", "pony"].includes(body.kind)) {
        return jsonResponse({ error: 'Le type doit \xEAtre "horse" ou "pony"' }, 400, getSecurityHeaders());
      }
      const horseData = {
        name: body.name.trim(),
        kind: body.kind,
    ...
    if (request.method === "PUT" && pathParts.length === 3) {
      ...
      const { data, error } = await db.from("horses").update(updateData).eq("id", horseId).select().single();
      if (error) return handleDbError(error);
      return jsonResponse(data, 200, getSecurityHeaders());
    }
    if (request.method === "DELETE" && pathParts.length === 3) {
      ...
      if (error) return handleDbError(error);
      return jsonResponse({ message: "Cheval supprim\xE9 avec succ\xE8s" }, 200, getSecurityHeaders());
    }
    return jsonResponse({ error: "Route non trouv\xE9e" }, 404, getSecurityHeaders());
  } catch (error) {
    console.error("Unexpected error:", error);
...
__name(handleHorses, "handleHorses");
async function handleHorseRiders(request, env, horseId) {
  const db = getDatabase(env);
  const clientIP = request.headers.get("CF-Connecting-IP") || "unknown";
  if (!checkRateLimit(clientIP, 60, 6e4)) {
    return jsonResponse({ error: "Trop de requ\xEAtes" }, 429, getSecurityHeaders());
  ...
  try {
    ...
    const { data, error } = await db.from("rider_horse_associations").select(`
      ...
      `).eq("horse_id", horseIdNum).order("association_start_date", { ascending: false });
    if (error) return handleDbError(error);
    return jsonResponse(data, 200, getSecurityHeaders());
  } catch (error) {
    console.error("Unexpected error:", error);
...
__name(handleHorseRiders, "handleHorseRiders");

// src/handlers/associations.js
init_checked_fetch();
init_modules_watch_stub();
async function handleAssociations(request, env) {
  const db = getDatabase(env);
  const url = new URL(request.url);
  const pathParts = url.pathname.split("/").filter(Boolean);
  const clientIP = request.headers.get("CF-Connecting-IP") || "unknown";
  ...
  try {
    ...
    if (request.method === "POST" && pathParts.length === 2) {
      ...
      const { data, error } = await db.from("rider_horse_associations").insert(associationData).select(`
          id,
          rider_id,
          horse_id,
          association_start_date,
    ...
    if (request.method === "PUT" && pathParts.length === 3) {
      ...
      const { data, error } = await db.from("rider_horse_associations").update(updateData).eq("id", associationId).select(`
          id,
          rider_id,
          horse_id,
          association_start_date,
    ...
    if (request.method === "DELETE" && pathParts.length === 3) {
      ...
      if (error) return handleDbError(error);
      return jsonResponse({ message: "Association supprim\xE9e avec succ\xE8s" }, 200, getSecurityHeaders());
    }
    return jsonResponse({ error: "Route non trouv\xE9e" }, 404, getSecurityHeaders());
  } catch (error) {
    console.error("Unexpected error:", error);
...
__name(handleAssociations, "handleAssociations");
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    console.log(`${method} ${path} - ${(/* @__PURE__ */ new Date()).toISOString()}`);
    if (method === "OPTIONS") {
